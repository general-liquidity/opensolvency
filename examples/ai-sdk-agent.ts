...
// Needs a model key (OPENSOLVENCY_MODEL_API_KEY). The model's `pay` tool executes
...
async function main(): Promise<void> {
  ...
  const apiKey = process.env.OPENSOLVENCY_MODEL_API_KEY ?? process.env[PROVIDER_API_KEY_ENV[provider]];
  if (!apiKey) {
    console.error("set OPENSOLVENCY_MODEL_API_KEY (and optionally OPENSOLVENCY_MODEL_PROVIDER).");
    process.exit(1);
  }
  const cfg = { provider, modelId: process.env.OPENSOLVENCY_MODEL ?? DEFAULT_MODEL_ID[provider], apiKey };
