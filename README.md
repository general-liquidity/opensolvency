...
| <img height="14" align="top" src="https://cdn.simpleicons.org/googlegemini/886FBF" />&nbsp; **AP2** | `@general-liquidity/opensolvency/ap2` | [AP2](https://github.com/google-agentic-commerce/ap2) interop - OS is the policy engine behind AP2: `Mandate ⇄ IntentMandate`, a signed `CartMandate` → a gated `PaymentIntent` (`gateAp2Cart`), merchant-JWT verification, A2A DataParts. |
| **AG-UI** | `@general-liquidity/opensolvency/agui` | An [AG-UI](https://ag-ui.com) surface - streams the gate's `confirm_operator` decision as a `confirm_spend` frontend tool-call to any AG-UI client (zero new deps). |
